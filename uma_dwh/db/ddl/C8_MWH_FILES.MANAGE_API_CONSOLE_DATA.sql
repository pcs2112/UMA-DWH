...
END else begin
 ...
 select 'D_REP_SKILL', rs.[REP_SKILL_displayName], rs.[REP_SKILL_UPDATE_TYPE] , rs.[REP_SKILL_SYSTEM] , rs.INSERT_DTTM, rs.[UPDATE_DTTM]
...
-- select * from [UMA_TELECOM].[D_REP_SKILL] where [ID] = 1;
 ...
 select 'D_REP_WORKGROUPS', rs.[REP_WORKGROUP_Name] , rs.[REP_WORKGROUP_id_ALTERNATE] , rs.INSERT_DTTM, rs.[UPDATE_DTTM]
 ...
 select 'D_REP_ROLES', rs.[REP_ROLE_roleId] , rs.[REP_ROLE_name] , rs.INSERT_DTTM, rs.[UPDATE_DTTM]
 from [UMA_TELECOM].[D_REP_ROLE] rs with(nolock) 
 order by [UPDATE_DTTM] desc;
 END;

 IF ( @message = 'LIST_REPS') BEGIN
 select 'D_REP', rs.[REP_ID] , rs.[REP_userId], rs.[REP_homeSite_ID] ,rs.[REP_firstName] ,rs.[REP_lastName] ,rs.[REP_displayName] ,rs.[REP_ntDomainUser] ,
